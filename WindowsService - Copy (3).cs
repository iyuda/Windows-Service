    ...
    class WindowsService : ServiceBase
    {
        ...
        public static HttpResponseMessage UrlRequest(string url, out string content, string method = "POST", string content_type = "application/json", string data = null)
        {
            try
            {
                ...
                string Authorization = WindowsService.config.GetProfileEntry("Settings", "Authorization", "Basic Q2Vtc1NlcnZpY2VBZG1pbjpkZjM1QCNZMkBDSWc =");
        ...
        public static string GetPcrReport(string pcr_id, string agency_id, string FaxNumber, string Agency)
        {
            try
            {
                string url = WindowsService.config.GetProfileEntry("Settings", "Web API URL For PCR Report", "http://www.creativeemstest.com:85/api/Couch/");
        ...
        public static void ValidatePcrId(string CouchUrl, string php_content, ref string couch_content, ref string pcr_id)
        {
            ...
            if (!String.IsNullOrEmpty(pcr_id) && was_empty)
            {
                ...
                string heap_url = GetHeapUrl(CouchUrl);
                UrlRequest(heap_url + "_pcr_id", out response_string, "PUT", data: "{" + "\"pcr_id\":\"" + pcr_id + "\"}");
        ...
        public static string GetHeapUrl(string OrigUrl)
        {
            string work_string = base_url;
            if (work_string.EndsWith("/"))
                work_string = work_string.Remove(work_string.Length - 1);

            string toReplace = work_string.Split('/')[work_string.Split('/').Length - 1];
            string HeapUrl = OrigUrl.Replace(toReplace, "app-heap");
            return HeapUrl;
        }
        public static bool SyncPcr(string CouchUrl, string PostFromCouchUrl, ref string couch_content, out string php_content, ref string pcr_id)
        {
            ...
            if (String.IsNullOrEmpty(pcr_id))
            {
                ...
                var groups = Regex.Match(base_url.ToString(), @"\/([^\}]+)\/").Groups;
                string heap_url = GetHeapUrl(CouchUrl);
                HttpResponseMessage response = UrlRequest(heap_url + "_pcr_id", out pcr_id_content, "GET");
            ...
            if (!bool_value)
            {
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    string sync_error = (JsonMaker.GetIOSJsonExtract("$.sync_error", couch_content) ?? "").ToString();
                    if (!String.IsNullOrEmpty(sync_error)) { 
                        JsonMaker.UpdateJsonValue("$.sync_error", "DELETE!", ref couch_content);
                        UrlRequest(CouchUrl, out response_string, "PUT", data: couch_content);
                    }
                }
        ...
        public static bool SendFaxReport(string pcr_id, string CouchUrl, string couch_content)
        {
            ...
            try
            {
                ...
                if (response.StatusCode == HttpStatusCode.OK)
                {
                    string pcr_status = (JsonMaker.GetIOSJsonExtract("$.pcr_status", couch_content) ?? "").ToString();
                     //if (pcr_status != "fax_error") return "";
                    ...
                    {
                        case "deleted":
                            DeletePcr(id, couch_content, pcr_status);
                            //rev = JsonMaker.GetIOSJsonExtract("$._rev", (object)couch_content);
                            //ValidatePcrId(CouchUrl, php_content, ref couch_content, ref pcr_id);
                        ...
                        case "inactive":
                            ...
                            {
                                Logger.LogActivity(PostFromCouchUrl + System.Environment.NewLine + php_content + System.Environment.NewLine + couch_content, "SyncErrorPcrs");
                                Logger.LogActivity(String.Format("Doc ID: {0}{1}PCR ID: {2}", id, System.Environment.NewLine, pcr_id), "SyncErrorPcrIds");
                                return PostFromCouchUrl + ":  " + php_content;
                            }
                        ...
                        case "sync_pending":
                            ...
                            JsonMaker.UpdateJsonValue("$.pcr_status", "synced", ref couch_content);
                            UrlRequest(CouchUrl, out response_string, "PUT", data: couch_content);
                            Logger.LogActivity(CouchUrl + "?rev=" + rev + " PUT Response: " + response_string);
                            break;
                        case "sync_error":
                            bool sync_retry_status = SetRetry(id, CouchUrl, "sync_retry");
                            if (sync_retry_status)
                                if (!SyncPcr(CouchUrl, PostFromCouchUrl, ref couch_content, out php_content, ref pcr_id))
                                {
                            ...
                            break;
                        case "fax_error":
                            bool fax_retry_status = SetRetry(id, CouchUrl, "fax_retry");
                            if (fax_retry_status)
                                SendFaxReport(pcr_id, CouchUrl, couch_content);
                            break;
                        default:
                            ...
                            else if (!String.IsNullOrEmpty(pcr_status) && !pcr_status.EndsWith("_error") && !pcr_status.ToLower().EndsWith("deleted"))
                            {
                                if (!SyncPcr(CouchUrl, PostFromCouchUrl, ref couch_content, out php_content, ref pcr_id))
                                {
                                    Logger.LogActivity(PostFromCouchUrl + System.Environment.NewLine + php_content + System.Environment.NewLine + couch_content, "SyncErrorPcrs");
        ...
        public static bool SetRetry(string id, string url, string field)
            {
                try
                {
                    string response_string;
                    //string url = CouchUrl;
                    UrlRequest(url, out response_string, "GET");
                    string couch_content = response_string;
                    //Logger.LogActivity(url + "SetRetry Get Response: " + response_string);
                    string max_retry = config.GetProfileEntry("Settings", field, "5");
                    int max_retry_int = int.Parse(max_retry);
                    int current_retry;
                    if (!int.TryParse((JsonMaker.GetIOSJsonExtract("$."+ field, couch_content) ??"0").ToString(), out current_retry))
                        current_retry = 0;

                    if (current_retry < max_retry_int)
                    {
                        current_retry++;
                        JsonMaker.UpdateJsonValue("$." + field, current_retry.ToString(), ref couch_content);
                        UrlRequest(url, out response_string,  "PUT", data: couch_content);
                        Logger.LogActivity(url + "SetRetry PUT Response: " + response_string);
                        return true;
                    }
                return false;
                }
        ...
        static void DeletePcrID_Doc(string doc_id)
        {
            ...
            //string base_url = config.GetProfileEntry("Settings", "CouchUrl", "http://localhost:5984/");
            //string url = base_url+ "/app-heap/" + doc_id + "_pcr_id";
            string url = GetHeapUrl(base_url)+"/" + doc_id + "_pcr_id";
        ...
        static void PutDeletePcr(string id, string couch_content, string pcr_status = "", string dateCreated = "")
        {
            string CouchUrl = config.GetProfileEntry("Settings", "CouchUrl", "http://192.168.1.60:5984/app-pcr/") + id;
            string rev = (JsonMaker.GetIOSJsonExtract("$._rev", couch_content) ?? "").ToString();
            string response_string;
            JToken token_content = (JToken)couch_content;

            //string ArchivePcrUrl = config.GetProfileEntry("Settings", "ArchivePcrUrl", "http://192.168.1.60:5984/archive-pcr/") + id;
            //UrlRequest(ArchivePcrUrl + "?rev=" + rev, out response_string, "PUT", data: JsonConvert.SerializeObject(token_content));

            //Logger.LogActivity(CouchUrl + "?rev=" + rev + " PUT DELETE Response: " + response_string);

            JsonMaker.UpdateJsonValue("$._deleted", true, ref token_content);
            UrlRequest(CouchUrl + "?rev=" + rev, out response_string, "PUT", data: JsonConvert.SerializeObject(token_content));
            if (pcr_status != "")
                response_string += System.Environment.NewLine + "pcr status = " + pcr_status;
            if (dateCreated != "")
                response_string += System.Environment.NewLine + "date created = " + dateCreated;

            Logger.LogActivity(CouchUrl + "?rev=" + rev + " PUT DELETE Response: " + response_string);
            DeletePcrID_Doc(id);
        }
        static void DeletePcr(string id, string couch_content,string pcr_status="",  string dateCreated = "")
        {
            PutDeletePcr(id, couch_content, pcr_status, dateCreated);
            return;
            ...
            //    response_string += System.Environment.NewLine + "pcr status = " + pcr_status;
            //if (dateCreated != "")
            //    response_string += System.Environment.NewLine + "pcr status = " + dateCreated;
        ...
        static void CheckUntouchedPcrs()
        {
            ...
            string OpenOrAttemptedTimeout = config.GetProfileEntry("Settings", "Open Or Attempted Timeout In Minutes", "1440");
                    ...
                    foreach (JToken SeqToken in SeqArray)
                    {
                        ...
                        if (response.StatusCode == HttpStatusCode.OK)
                        {
                            ...
                            if (pcr_status == "deleted")
                            {
                                ...
                                DeletePcr(id, couch_content, pcr_status: pcr_status);
                            }
                            else
                            {
                                string dateCreated = (JsonMaker.GetIOSJsonExtract("$.dateCreated", couch_content) ?? "").ToString();
                                if (Utilities.IsDate(dateCreated))
                                {
                                    switch (pcr_status)
                                    {
                                        case "untouched":
                                            if (DateTime.Parse(dateCreated).AddMinutes(intUntouchedTimeout) < DateTime.Now)
                                                DeletePcr(id, couch_content, pcr_status: pcr_status, dateCreated: dateCreated);
                                            break;
                                        case "opened":
                                        case "attempted":
                                        case "fax_error":
                                        case "synced":
                                        case "faxed":
                                            if (DateTime.Parse(dateCreated).AddHours(24) < DateTime.Now)
                                                DeletePcr(id, couch_content, pcr_status: pcr_status, dateCreated: dateCreated);
                                            break;
                                        case "inactive":    
                                        case "sync_pending":
                                ...
                                //    bool condition4 = ((pcr_status == "synced" || pcr_status == "faxed") && DateTime.Parse(dateCreated).AddMinutes(720) < DateTime.Now);
                                //    bool condition5 = (pcr_status == "synced" || pcr_status == "faxed");
                                //    if ((condition2 || condition3) && !condition5  && pcr_status != "untouched")
                                //    {
                                //        string PostFromCouchUrl = config.GetProfileEntry("Settings", "PostFromCouchUrl", "http://creativeemstest.com/cemslocal/yiicems/index.php?r=api/CouchDB/PostPcrFromCouch");
                                //        string php_content;
                                //        string pcr_id = (JsonMaker.GetIOSJsonExtract("$.pcr_id", couch_content) ?? "").ToString();
                                ...
                                //        //string ConditionMessage = "dateCreated = " + dateCreated;
                                //        //ConditionMessage += condition1 ? ", condition1" : "";
                                //        //ConditionMessage += condition2 ? ", condition2" : "";
                                //        //ConditionMessage += condition3 ? ", condition3" : "";
                                //        //ConditionMessage += condition4 ? ", condition4" : "";
